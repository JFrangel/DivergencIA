            ...
            return (
              <motion.div key={m.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
                <Link to={`/members/${m.id}`}>
                  <motion.div
                    className="glass rounded-2xl p-5 flex flex-col items-center text-center gap-3 cursor-pointer relative overflow-hidden group"
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/35 mt-0.5 truncate max-w-[140px]">{m.carrera || 'Investigador'}</p>
                    </div>
                    <div className="flex flex-wrap justify-center gap-1">
                      {m.area_investigacion && <Badge area={m.area_investigacion} size="xs" />}
                      {m.es_fundador && <Badge variant="founder" size="xs" />}
                    </div>
                    {m.bio && <p className="text-[11px] text-white/25 leading-relaxed line-clamp-2">{m.bio}</p>}
                  </motion.div>
                </Link>
              </motion.div>
            )
