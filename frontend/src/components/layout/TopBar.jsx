    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <button
            className="relative w-8 h-8 flex items-center justify-center rounded-lg text-white/30 hover:text-[#00D1FF] hover:bg-[#00D1FF]/5 transition-all"
          ...
          >
            ...
            {unreadCount > 0 && (
              <motion.span
                className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center bg-[#FC651F] text-white"
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
            ...
            {showNotifs && (
              ...
              >
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
                  ...
                  {unreadCount > 0 && (
                    <button onClick={markAllRead} className="text-[10px] text-[#00D1FF] hover:text-[#00D1FF]/80 transition-colors flex items-center gap-1">
                ...
                <div className="max-h-64 overflow-y-auto">
                  {notifications.length === 0 ? (
                    ...
                    notifications.slice(0, 10).map(n => (
                      <div
                        key={n.id}
                        className={`px-4 py-3 border-b border-white/[0.04] hover:bg-white/[0.03] cursor-pointer transition-colors ${!n.leida ? 'bg-[#FC651F]/5' : ''}`}
